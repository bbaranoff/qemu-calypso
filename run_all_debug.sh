...
[ -f "$HACK" ]   || { echo "FATAL: $HACK not found"; exit 1; }
[ -f "$CFILE" ]  || { echo "FATAL: $CFILE not found"; exit 1; }

# hack_gdb.py BP address mode:
#   HACK_DISCOVER=1                 → resolve from layer1.highram.elf symbol table
#   HACK_BP_FBDET=0xADDR            → explicit override for FB-det BP
#   HACK_BP_FBCOMPL=0xADDR          → explicit override for FB-compl BP
#   (none)                          → hardcoded defaults baked into hack_gdb.py
if [ "${HACK_DISCOVER:-}" = "1" ]; then
    HACK_MODE="discover"
elif [ -n "${HACK_BP_FBDET:-}" ] || [ -n "${HACK_BP_FBCOMPL:-}" ]; then
    HACK_MODE="env-override"
else
    HACK_MODE="hardcoded-defaults"
fi
export HACK_DISCOVER HACK_BP_FBDET HACK_BP_FBCOMPL HACK_FW

echo "[run_all] launching $NRUNS runs (kill at ${QEMU_LOG_MAX} bytes or ${RUN_TIMEOUT}s)"
echo "[run_all] hack BP mode: $HACK_MODE"
...
for k in $(seq 1 "$NRUNS"); do
    ...
    # hack.py (releases QEMU from -S, sets BPs, forces FBSB)
    ( sleep 2; python3 -u "$HACK" > "$HACK_LOG" 2>&1 ) &
    HACK_PID=$!
