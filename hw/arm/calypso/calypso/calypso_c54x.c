...
int c54x_run(C54xState *s, int n_insns)
{
    ...
    while (executed < n_insns && s->running && !s->idle) {
        ...
        /* Detect unexpected SP changes (push without pop) */
        if (s->sp != sp_before) {
            static int sp_leak_log = 0;
            if (sp_leak_log < 200) {
                C54_LOG("SP %+d PC=0x%04x op=0x%04x SP 0x%04x→0x%04x",
                        (int16_t)(s->sp - sp_before), exec_pc, exec_op, sp_before, s->sp);
                sp_leak_log++;
            }
        }
