...
DEFAULT_BP_FREQ_BGE   = 0x008265bc  # bge 0x8265d8 — skip to fall-through
DEFAULT_BP_SNR_CMP    = 0x008265c4  # cmp r3, #0 — patch r3=1 so bne taken
...
def main() -> int:
   ...
   └────────────────────────────────────────────────────────────┘
    ...
    print(f"[hack]   ✓ HW#4  {BP_FBSB_COMPL:#010x}  l1a_fb_compl+0x08    →  r3 := 0   (force attempt < 13  ⇒  FBSB result=0)")
    print(f"[hack] ░ state engaged: [FB-FORCE] [FREQ-SKIP] [SNR-FORCE] [COMPL-OVERRIDE]")
    ...
    while True:
        ...
        if pc == BP_FBDET_RESP:
            ...
            print(
                ...
                f"SNR={n_snr_force} COMPL={n_compl_force} "
